-- Atualiza create_checkout_order para receber e salvar flag de "Compre junto"
...
BEGIN
  ...
  IF p_user_id IS NOT NULL AND p_address_id IS NOT NULL THEN
    INSERT INTO orders (user_id, status, subtotal, shipping_price, shipping_method_id, shipping_method_name, discount_amount, total, address_id, shipping_address, payment_status, used_buy_together)
    VALUES (p_user_id, 'pending', v_subtotal, v_shipping_price, v_method.id, v_method.name, v_discount, v_total, p_address_id, v_shipping_json, 'pending', p_used_buy_together)
    RETURNING id INTO v_order_id;
  ELSE
    INSERT INTO orders (user_id, status, subtotal, shipping_price, shipping_method_id, shipping_method_name, discount_amount, total, shipping_address, customer_name, customer_email, customer_phone, guest_access_token, payment_status, used_buy_together)
    VALUES (p_user_id, 'pending', v_subtotal, v_shipping_price, v_method.id, v_method.name, v_discount, v_total, v_shipping_json, v_customer_name, v_customer_email, v_customer_phone, v_guest_token, 'pending', p_used_buy_together)
    RETURNING id INTO v_order_id;
  END IF;
  FOR v_item IN SELECT value FROM jsonb_array_elements(p_items) AS t(value) LOOP
    v_product_id := (v_item->>'product_id')::UUID;
