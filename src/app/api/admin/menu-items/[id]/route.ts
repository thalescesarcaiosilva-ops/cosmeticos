...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  if (error || !data) {
    return jsonError('Não foi possível atualizar o item do menu', 400)
  }

  revalidateTag('site-layout', 'max')

  return jsonSuccess(data, 'Item do menu atualizado')
}
...
export async function DELETE(_request: Request, context: RouteContext) {
  ...
  if (error) {
    return jsonError('Não foi possível remover o item do menu', 400)
  }

  revalidateTag('site-layout', 'max')

  return jsonSuccess({ ok: true }, 'Item do menu removido')
}
