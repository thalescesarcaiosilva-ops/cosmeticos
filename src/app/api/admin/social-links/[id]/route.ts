...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  if (error || !data) {
    return jsonError('Não foi possível atualizar o link social', 400)
  }

  revalidateTag('site-layout', 'max')

  return jsonSuccess(data, 'Link social atualizado')
}
...
export async function DELETE(_request: Request, context: RouteContext) {
  ...
  if (error) {
    return jsonError('Não foi possível remover o link social', 400)
  }

  revalidateTag('site-layout', 'max')

  return jsonSuccess({ ok: true }, 'Link social removido')
}
